    ...
    if decision == "***Enter or Paste Values***":
            ...
            if load or st.session_state.load_state:
                ...
                # Read the clipboard content into a pandas DataFrame
                    ...
                    if df.shape[1] == 2:  
                        df.columns = ['Gp(bscf)', 'P_avg(psia)']  # Assign default column names
                        st.success("Data pasted successfully!")
                    else:
                        st.warning("Clipboard data must contain exactly 2 columns. Please re-check.")
                    # Allow editing of pasted data
                ...
                st.info("Waiting for you to paste the data / Manually input the data in the table below...")
